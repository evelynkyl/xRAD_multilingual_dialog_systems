...
from parlai.agents.cora.args import WIKIPEDIA_COMPRESSED_INDEX, WIKIPEDIA_EXACT_INDEX, MULTILINGUAL_WIKIPEDIA_COMPRESSED_INDEX, MULTILINGUAL_WIKIPEDIA_EXACT_INDEX
...
class BaseIndexer(ABC):
    ...
    def search(
        ...
        logging.info(f'query_vectors {query_vectors.shape}')
        _scores, indexes, vectors = self.index.search_and_reconstruct(
            query_vectors, top_docs
        )
        print("this is the scores, index, vectors", _scores, indexes, vectors)
        db_ids = [
...
class CompressedIndexer(BaseIndexer):
    ...
    def __init__(self, opt: Opt):
        ...
        if self.index_factory:
            logging.info(f'Creating Index from Index Factory: {self.index_factory}')
...
def indexer_factory(opt: Opt) -> BaseIndexer:
    ...
    if opt['indexer_type'] == 'compressed':
        if opt['path_to_index'] == MULTILINGUAL_WIKIPEDIA_EXACT_INDEX:
            logging.warning(
                f'Changing index path to compressed index: {MULTILINGUAL_WIKIPEDIA_COMPRESSED_INDEX}'
            )
            opt['path_to_index'] = "/root/cora/emb_index/IVF4096_HNSW128__PQ128.index"  # modelzoo_path(
         #       opt['datapath'], MULTILINGUAL_WIKIPEDIA_COMPRESSED_INDEX
          #  )
        indexer = CompressedIndexer(opt)
    elif opt['indexer_type'] == 'exact':
        if opt['path_to_index'] == MULTILINGUAL_WIKIPEDIA_COMPRESSED_INDEX:
            logging.warning(
                f'Changing index path to exact index: {MULTILINGUAL_WIKIPEDIA_EXACT_INDEX}'
            )
            opt['path_to_index'] = modelzoo_path(opt['datapath'], MULTILINGUAL_WIKIPEDIA_EXACT_INDEX)
