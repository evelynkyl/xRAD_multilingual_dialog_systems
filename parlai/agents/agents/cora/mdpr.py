...
from parlai_internal.agents.hugging_face.dict import HuggingFaceDictionaryAgent
from parlai.agents.cora.args import mDPR_MODEL
from parlai.agents.cora.conversion_utils import BertConversionUtils
...
@register_agent('mdpr_agent')
class mDPRAgent(TransformerRankerAgent):
    """
    TRA Wrapper for the mDPR Models.
    """

    def __init__(self, opt, shared=None):
        config: BertConfig = BertConfig.from_pretrained('bert-base-multilingual-uncased')
    ...
    def build_model(self):
        return mDPR_MODEL(self.opt, self.dict)
    ...
    def build_dictionary(self):
        ...
        """
        return mBertTokenizerDictionaryAgent(self.opt)


class mDprEncoder(TransformerEncoder):
    """
    ...
    def __init__(
        self,
        opt: Opt,
        dpr_model: str = 'mbert_from_cora',
        pretrained_path: str = mDPR_MODEL,
        ...
        try:
            ...
            config_path = PathManager.get_local_path(
                    os.path.join(opt['datapath'], "bert-base-multilingual-uncased", self.CONFIG_PATH)
            )
            config: BertConfig = BertConfig.from_pretrained(config_path)

        pretrained_path = "/root/cora/mDPR_biencoder_best/mDPR_biencoder_best.cpt"  # modelzoo_path(
     #       opt['datapath'], pretrained_path
      #  )  # type: ignore
        if not os.path.exists(pretrained_path):
            # when initializing from parlai rag models, the pretrained path
            # may no longer exist. This is fine if we've already trained
            # the model.
            assert dpr_model == 'mbert_from_cora' #changed from bert_from_parlai_rag
            logging.error(f'Pretrained Path does not exist: {pretrained_path}')
            pretrained_path = modelzoo_path(
                opt['datapath'], mDPR_MODEL
            )  # type: ignore
            dpr_model = 'mbert_from_cora'
        ...
        logging.info("Succesfully loaded the pretrained model state dict!")
    ...
    def _load_state(
        ...
        if dpr_model == 'bert':
            ...
            self.load_state_dict(state_dict)
        elif dpr_model == 'mbert_from_cora':
            logging.info(f"Lading previously trained model from {pretrained_path}")
        #    state_dict = torch.load(pretrained_path, 
         #                   map_location=lambda s, l:default_restore_location(s, 'cpu'))
            ...
            self.load_state_dict(state_dict)
            print("this is the loaded state dict", state_dict)
       #    key = f"{encoder_type}_encoder."
            #print("model state dict keys are %s", state_dict.keys())
          #  print(f"this is state dict {state_dict}")
            #if "model_dict" in state_dict:
             #   print("FOUND model state dict!!!")

           # prefix = 'question_model.' if encoder_type == 'query' else 'ctx_model.'
            #prefix_len=len(prefix)
            #print("this is the prefix", prefix)
            #question_encoder_state = {key[prefix_len:]: value
             #   for (key,value) in state_dict['model_dict'].items()
             #   if key.startswith("question_model.")
            #}
            #question_encoder_state.update(
             #                   {k: v for k, v in state_dict['model_dict'].items() 
              #                  if 'encode_proj' in k}
               #                 )
           # try:
            #    logging.info(f"loading state dict")
             #   self.load_state_dict(question_encoder_state)
              #  vector_size = self.get_out_size()
              #  logging.info(f'Encoder vector_size: {vector_size}')
             #   logging.info("encoder state dict is loaded!")
           # except RuntimeError:
            #    for key in ['embeddings.position_ids']: #BERT_COMPATIBILITY_KEYS    
             #       question_encoder_state[key] = recursive_getattr(self, key)
             #   self.load_state_dict(question_encoder_state)
        #    prefix_len = len('question_model.')
         #   state_dict = {
          #      key[prefix_len:]: value for (key, value) in state_dict.items() if key.startswith('question_model.')
           # }
           # self.load_state_dict(state_dict)


class mDprQueryEncoder(mDprEncoder):
    """
    Query Encoder for mDPR.
    """
...
class mDprDocumentEncoder(mDprEncoder):
    """
    Document Encoder for mDPR.
    """
...
class mDPRModel(TransformerMemNetModel):
    """
    ...
    def _get_build_options(cls, opt: Opt):
        ...
        """
        query_model = 'mbert_from_cora'
        document_model = 'mbert_from_cora'
    ...
    def build_context_encoder(
        ...
        return mDprQueryEncoder(opt, dpr_model=query_model, pretrained_path=query_path)
    ...
    def build_candidate_encoder(
        ...
        return mDprDocumentEncoder(
            opt, dpr_model=document_model, pretrained_path=document_path
        )


class mBertTokenizerDictionaryAgent(HuggingFaceDictionaryAgent):
