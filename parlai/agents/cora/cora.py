...
Original Paper: https://arxiv.org/abs/2107.11976
...
from parlai_internal.agents.mgen.mgen import MgenAgent
...
from parlai.agents.cora.args import setup_rag_args
from parlai.agents.cora.model_types import (
    RagModelInterface,
    RagSequence,
    RagToken,
    RagTurn,
)
from parlai.agents.cora.modules import CoraModel, T5RagModel, mGENRagModel
from parlai.agents.cora.retrievers import Document
from parlai.utils.fsdp import is_fsdp
from parlai.agents.cora.translator import (
    detect_language,
...
class BaseGenerationAgentMixin(ABC):
    ...
    def build_rag_model(opt: Opt, dictionary: DictionaryAgent) -> CoraModel:
        """
...
class TransformerGeneratorRagAgent(TransformerGeneratorAgent, BaseGenerationAgentMixin):
    @staticmethod
    def build_rag_model(opt: Opt, dictionary: DictionaryAgent) -> CoraModel:
        return CoraModel(opt, dictionary)
...
class mGENRagAgent(MgenAgent, BaseGenerationAgentMixin):
    @staticmethod
    def build_rag_model(opt: Opt, dictionary: DictionaryAgent) -> mGENRagModel:
        return mGENRagModel(opt, dictionary)
    ...
    def _generate(
        ...
        """
        Override since mGEN/mT5 needs to call TGA generate.
        """
        return TorchGeneratorAgent._generate(
            self, batch, beam_size, max_ts, prefix_tokens
        )
...
class CoraAgent(TransformerGeneratorRagAgent, BartRagAgent, T5RagAgent, mGENRagAgent):
    """
    RagAgent.
    ...
    _generation_agent: Union[
        Type[T5RagAgent], Type[mGENRagAgent], Type[BartRagAgent], Type[TransformerGeneratorRagAgent]
    ]
    ...
    def add_cmdline_args(
        ...
        mGENRagAgent.add_cmdline_args(parser, partial_opt=partial_opt)
    ...
    def build_regret_model(self) -> CoraModel:
        """
        Build and return regret CoraModel.
        """
        ...
        if model_file:
            ...
            model = CoraModel(regret_opt, regret_dict, retriever_shared=retriever_shared)
    ...
    def observe(self, observation: Union[Dict, Message]) -> Message:
        ...
        print(f"this is an observation: {observation}")
    ...
    def eval_step(self, batch: Batch) -> Optional[Output]:
        ...
        if output is None or not hasattr(self.model, 'retriever'):
            print(f"don't have retriever or ouput! so this is the output! {output}")
            return output
        if hasattr(self.model.retriever, 'top_docs'):  # type: ignore
            print(f"got top docs in the output: {output}")
    ...
    def build_model(self) -> CoraModel:
        """
        Build and return CoraModel.
        """
    ...
    def _get_initial_decoder_input(
        ...
        print("this is rag decoder input", rag_dec_input)
    ...
    def _get_next_decoder_input(
        ...
        print("this is decoder input", dec_input)
        return dec_input
    ...
    def _generate(
        ...
        print("this is the generation output:", gen_outs)
        return gen_outs
    ...
    def _rag_generate(
        ...
        rag_out =  self._generation_agent._generate(
            self, batch, beam_size, max_ts, prefix_tokens
        )
        print("this is the rag_out generation:", rag_out)
        return rag_out
    ...
    def get_model_output(self, batch: Batch) -> Tuple[Any, ...]:
        ...
        print("this is preds from model output:", preds)
