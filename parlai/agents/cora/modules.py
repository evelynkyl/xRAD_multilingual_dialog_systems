...
"""
Modules for CORA, copy from RAG.
"""
import torch
import torch.cuda
import torch.nn
...
from parlai_internal.agents.mgen.mgen  import (
    ParlaimGENEncoder,
    ParlaimGENDecoder,
    build_mGEN,
    set_device,
...
from parlai.agents.cora.retrievers import retriever_factory, Document
...
class CoraModel(TorchGeneratorModel):
    """
    RagModel.
    ...
    def __init__(self, opt, dictionary, retriever_shared=None):
        ...
        print("this is the retriever: ", opt)
    ...
    def encoder(
       ...
       # logging.info("retrieve, get the expanded input")
        if all([tensor is not None for tensor in [input_lengths, query_vec]]):
            print("found the tensors, now attempting retrieve and concat")
     ...
     #   logging.info("got the expanded input")

        # Run through seq2seq encoder
      #  logging.info("running through the mgen encoder")
    ...
    def decoder(
      ...
      #  logging.info("getting decoder outputs")
      ...
      #  print("this is deoder output: ", dec_out)

        if all([obj is not None for obj in [docs, doc_scores]]):
            # 2. Get logprobs
            logging.info("getting log probs")
           ...
           # print(f"this is the out probs: {out_probs}")
            # 3. Marginalize
            print("getting marginalize")
            marginalized = self._rag_model_interface.marginalize(
                out_probs, F.log_softmax(doc_scores, dim=1), input_turns_cnt
            )
         #   print(f"this is the output of marginazlied: {marginalized}")
        else:
            # With RAG Sequence Generation, we do not marginalize over documents.
            logging.info("using rag seq geneneration, do not need to marginalzie over docs")
            marginalized = dec_out
           # print("this is the decoded output: ", marginalized)
    ...
    def retrieve_and_concat(
        ...
        """
        # 1. Retrieve
        logging.info("retrieve top docs")
       # print("this is the retriever", self.retriever)
        top_docs, top_doc_scores = self.retriever.retrieve(query_vec)
        print("this is the top docs ", top_docs)
        
        # 2. Expand the input
        print("expanding the input")
        if input_turns_cnt is not None:
            print("found input_turns_cnt")
        ...
        print("this is the expanded_input", expanded_input)
        return expanded_input, top_docs, top_doc_scores
...
class T5RagModel(CoraModel):
    ...
    def build_encoder(
        ...
        **kwargs,
        ...
        return CoraModel.build_encoder(
            opt,
    ...
    def build_decoder(
        ...
        **kwargs,
        ...
        return CoraModel.build_decoder(
            opt,
...
class mGENRagModel(CoraModel):
    """
    mGEN/mT5 For RAG.
    """

    def __init__(self, opt, dictionary, retriever_shared=None):
        opt['mGEN'] = build_mGEN(opt)
        if opt['mGEN_model_parallel']:
            opt['mGEN'].parallelize()
        else:
            opt['mGEN'].deparallelize()
        super().__init__(opt, dictionary, retriever_shared)
        self.embedding_size = opt['mGEN'].model_dim
        self.mGEN = opt.pop('mGEN', None)
    ...
    def build_encoder(
        ...
        **kwargs,
        ...
        return CoraModel.build_encoder(
            opt,
            encoder=opt['mGEN'].get_encoder(),
            encoder_class=ParlaimGENEncoder,
            **kwargs,
        )
    ...
    def build_decoder(
        ...
        **kwargs,
        ...
        return CoraModel.build_decoder(
            opt,
            decoder=opt['mGEN'].get_decoder(),
            decoder_class=ParlaimGENDecoder,
            **kwargs,
        )
    ...
    @set_device
    def decoder_output(self, latent: torch.Tensor):
        tensor = latent * (self.mGEN.model_dim ** -0.5)
        logits = self.mGEN.lm_head(tensor)
        return logits
