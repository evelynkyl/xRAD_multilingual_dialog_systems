...
class BaseModelChatBlueprint(ParlAIChatBlueprint, ABC):
    ...
    def format_left_pane_text(self, args: "DictConfig"):
        ...
        self.left_pane_text = self.left_pane_text.format(
            lang_in_use=args.blueprint.lang_in_use)
    ...
    def get_frontend_args(self) -> Dict[str, Any]:
        ...
        if self.args.blueprint.get('annotations_config_path', '') != '':
            logging.info(f"reading {self.args.blueprint.annotations_config_path}")
            with open(
                self.args.blueprint.annotations_config_path, "r", encoding="utf-8-sig"
            ) as f:
            #    print(f"reading {self.args.blueprint.annotations_config_path}")
...
class ModelChatBlueprintArgs(BaseModelChatBlueprintArgs):
    ...
    conversation_start_mode: str = field(
        default='hi_diff_lang',
        metadata={
            "help": 'Set to "hi_diff_lang" to show "Hi!" in a non-English language'
    ...
    start_language: str = field(
        default='ko', 
        metadata={"help": "Select a language to start with"}
    )
    lang_in_use: str = field(
        default='Korean',
        metadata={"help": "Name of the language to be used, for the website's instruction."}
    )
    ...
    max_onboard_time: int = field(
        default=600, metadata={"help": "time limit accepting onboarding"}
    )
...
class ModelChatBlueprint(BaseModelChatBlueprint):
    ...
    def __init__(
        ...
        if (
            ...
            if args.blueprint.conversation_start_mode == 'hi' or args.blueprint.conversation_start_mode == 'hi_diff_lang':
        ...
        shared_state.world_opt.update(
            {
                ...
                'start_language':args.blueprint.start_language,
            }
        )
    ...
    def _get_shared_models(self, args: "DictConfig") -> Dict[str, dict]:
        ...
        return TranslatorTurkLikeAgent.get_bot_agents(args=args, model_opts=active_model_opts, no_cuda=True)


@dataclass
class TranslateModelChatBlueprintArgs(BaseModelChatBlueprintArgs):
    _blueprint_type: str = BLUEPRINT_TYPE
    _group: str = field(
        default="TranslateModelChatBlueprint",
        metadata={
            'help': "This task adds a translator in the input and output of the dialogue, runs conversations between a human and one of a set of "
            "provided models, asking workers to evaluate individual turns and "
            "the overall model quality."
        },
    )
...
@register_mephisto_abstraction()
class TranslateModelChatBlueprint(BaseModelChatBlueprint):
    """
    Blueprint for model chat without images.
    ...
    ArgsClass = TranslateModelChatBlueprintArgs
    SharedStateClass = SharedModelChatTaskState
    BLUEPRINT_TYPE = BLUEPRINT_TYPE
