...
class ModelChatWorld(BaseModelChatWorld):
    ...
    def _run_initial_turn(self) -> None:
        ...
        if self.opt['conversation_start_mode'] == 'blended_skill_talk':
            ...
            """
            NEW GREETINGS FOR MULTILINGUAL! 
            """
        elif self.opt['conversation_start_mode'] == 'hi_diff_lang':
            print('[Displaying "Hi!" in a different language only as per Meena task.]')
            if self.personas is not None:
                human_persona_strings = [s.strip() for s in self.personas[0]]
            else:
                human_persona_strings = ['', '']
            ...
            print(f"got {self.opt.get('start_language', None)} as the input language")
            ...
            if self.opt['start_language'] in hello_in_five_lang:
                ...
                lang_in_use = lang_match[self.opt['start_language']]
            else:
                print(f"cant find {self.opt.get('start_language', None)} in the input language")
...
def get_translate_bot_worker(opt: Dict[str, Any], model_name: str) -> TranslatorTurkLikeAgent:
    """
    Return a bot agent.
    ...
    num_turns = opt['num_turns']
    bot_agent = create_agent_from_shared(shared_bot_agents[model_name])
    bot_worker = TranslatorTurkLikeAgent(
        opt,
        model_name=model_name,
        model_agent=bot_agent,
        num_turns=num_turns,
...
def make_world(opt, agents):
    ...
    if model_name == "blender_400M":
        # machine translation component for the baseline model
        bot_worker = get_translate_bot_worker(opt=opt, model_name=model_name)
    else:    
