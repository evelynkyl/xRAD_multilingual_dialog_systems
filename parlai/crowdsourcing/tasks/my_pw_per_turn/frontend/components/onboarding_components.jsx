...
const ONBOARDING_MAX_FAILURES_ALLOWED = 10;
...
var renderOnboardingFail = function() {
  ...
  alert(
    "Sorry, you've exceeded the maximum amount of tries to label the sample conversation correctly, and thus we don't believe you can complete the task correctly. Please return the HIT."
  );
};
...
var handleOnboardingSubmit = function({
  onboardingData,
  ...
  if (
    countCorrect >= ONBOARDING_MIN_CORRECT &&
    countIncorrect <= ONBOARDING_MAX_INCORRECT
  ) {
    ...
    if (onboardingFailuresCount < ONBOARDING_MAX_FAILURES_ALLOWED) {
      ...
      alert(
        "You did not label the sample conversation well enough. Please try one more time!"
      );
    } else {
...
function OnboardingUtterance({
  ...
  if (turnIdx % 2 == 1) {
    ...
    return (
      <div
        ...
        style={{ float: `left`, display: "table", clear: `both` }}
      >
        <div className="onboarding-text">
          ...
          <br />
          <b>Response 3:</b> {text_response_3}
  ...
  return (
    <div
      ...
      style={{ float: `right`, display: "table", clear: `both` }}
    >
...
function OnboardingComponent({
  ...
  if (onboardingData === null) {
    ...
    const [currentTurnAnnotations, setCurrentAnnotations] = React.useState(
      Array.from(Array(onboardingData.dialog.length), () =>
        Object.fromEntries(
    ...
    return (
      <div id="onboarding-main-pane">
        <OnboardingDirections>
          ...
          <div>
            To first learn about the evaluation task, please choose the correct
            checkbox for the given question (in italics) for this conversation.
          </div>
        </OnboardingDirections>
              ...
              {onboardingData.dialog.map((turn, idx) => (
                <div key={"turn_pair_" + idx}>
                  ...
                  <OnboardingUtterance
                    ...
                    text_response_3={turn[1].text_response_3}
                    annotations={currentTurnAnnotations[idx]}
                    onUpdateAnnotation={newAnnotations => {
