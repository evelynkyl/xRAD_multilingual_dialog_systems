...
function CheckboxTextResponse({
  ...
  const middle_bot_data = taskContext.middle_bot_data;
  const bottom_bot_data = taskContext.bottom_bot_data;

  const top_bot_id = top_bot_data === undefined ? "" : top_bot_data.top_bot_id;
  const middle_bot_id =
    middle_bot_data === undefined ? "" : middle_bot_data.middle_bot_id;
  const bottom_bot_id =
    bottom_bot_data === undefined ? "" : bottom_bot_data.bottom_bot_id;
  const top_bot_response =
    top_bot_data === undefined ? null : top_bot_data.top_bot_response;

  const middle_bot_response =
    middle_bot_data === undefined ? null : middle_bot_data.middle_bot_response;

  const bottom_bot_response =
    bottom_bot_data === undefined ? null : bottom_bot_data.bottom_bot_response;
  const top_bot_response_text =
    top_bot_data === undefined ? "" : top_bot_data.top_bot_response.text;
  const middle_bot_response_text =
    middle_bot_data === undefined
      ? ""
      : middle_bot_data.middle_bot_response.text;
  const bottom_bot_response_text =
    bottom_bot_data === undefined
      ? ""
      : bottom_bot_data.bottom_bot_response.text;

  const task_turn_idx =
    taskContext.task_turn_idx === undefined ? -1 : taskContext.task_turn_idx;
      ...
      onMessageSend({
        ...
        task_data: {
          ...
          not_accepted_bot_response1:
            chosenBotResponseValue === top_bot_response
              ? bottom_bot_response
              : top_bot_response,
          ...
          not_accepted_bot_id1:
            chosenBotIdValue === top_bot_id ? bottom_bot_id : top_bot_id,
          not_accepted_bot_id2:
            chosenBotIdValue === top_bot_id ? middle_bot_id : top_bot_id,
          task_turn_idx: task_turn_idx,
  ...
  const tryJustificationSend = React.useCallback(() => {
    ...
    if (Object.keys(chosenBotResponseValue).length == 0) {
      alert(
        "Please choose a response and write your justification before proceeding!"
      );
  ...
  const handleRadioChange = e => {
    ...
    if (e.target.value === top_bot_response_text) {
    ...
    } else if (e.target.value === middle_bot_response_text) {
      setChosenBotResponseValue(middle_bot_response);
      setChosenBotIdValue(middle_bot_id);
      ...
      alert(
        "Something went extremely wrong. Please contact the requestors reporting this error."
      );
    }
  };
  ...
  if (
    task_turn_idx !== -1 &&
    task_turn_idx >= 2 * (taskConfig.min_num_turns - 1)
  ) {
    /*
    ...
    return (
      <div className="response-type-module">
        ...
        <FormGroup>
          ...
          <Radio
            name="middle_bot_response"
            value={middle_bot_response_text}
            onChange={handleRadioChange}
            checked={chosenBotResponseValue === middle_bot_response}
          >
            {middle_bot_response_text}
          </Radio>
          <Radio
        ...
        <div>
          <b>
            Please provide a brief justification for your choice (a few words or
            a sentence):
          </b>
        </div>
    ...
    if (task_turn_idx !== expectedTurnValue) {
      ...
      return (
        <div className="response-type-module">
          ...
          <div>
            <b>
              Please provide a brief justification for your choice (a few words
              or a sentence):
            </b>
          </div>
