...
from parlai.crowdsourcing.tasks.my_pw_per_turn.bot_agent import (
    PerTurnEvalTurkLikeAgent,
    TranslatorPerTurnEvalTurkLikeAgent,
)
...
BLUEPRINT_TYPE = 'my_per_turn_eval_blueprint'


@dataclass
class MyPerTurnEvalBlueprintArgs(ModelChatBlueprintArgs):
    ...
    block_on_onboarding_fail: bool = field(
        default=False, metadata={"help": "whether to block on onboarding failure"}
    )

@register_mephisto_abstraction()
class MyPerTurnEvalBlueprint(ModelChatBlueprint):
    """
    ...
    ArgsClass = MyPerTurnEvalBlueprintArgs
    ...
    def assert_task_args(
        ...
        if (
            ...
            try:
                ...
                for part in parts:
                    model1, model2, model3, num_string = part.split(':')
                    models_alphabetize = [model1, model2, model3]
                    models_alphabetize.sort()
                    conversations_needed[
                        f'{models_alphabetize[0]}:{models_alphabetize[1]}:{models_alphabetize[2]}'
                    ] = int(num_string)
            except Exception as e:
                raise Exception(
                    "Could not create conversations needed dict from given string. "
                    f"Error was {e}.\n"
                    "Be sure the format is like modelA:modelB:50:modelC:20,modelD:modelE:50:modelD:20"
                )
        else:
    ...
    def _process_conversations_needed(self, args: "DictConfig") -> Dict[str, int]:
        ...
        for part in parts:
            ...
            conversations_needed[
                ...
                num_string
            )  # format is model_1_name:model_2_name:model_3_name
    ...
    def _get_shared_models(self, args: "DictConfig") -> Dict[str, dict]:
        ...
        return TranslatorPerTurnEvalTurkLikeAgent.get_bot_agents(
            args=args, model_opts=active_model_opts,no_cuda=True
        )
