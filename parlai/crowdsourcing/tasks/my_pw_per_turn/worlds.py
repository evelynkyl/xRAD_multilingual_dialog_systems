...
from parlai.crowdsourcing.tasks.my_pw_per_turn.bot_agent import PerTurnEvalTurkLikeAgent, TranslatorPerTurnEvalTurkLikeAgent
...
#db = LocalMephistoDB()
#task = db.find_tasks(task_name='per_turn_eval__engaging')[0]
#onboarding_agents = db.find_onboarding_agents(task_id=task.db_id)
#print("this is onboarding_agent ", onboarding_agents[-1].state.get_data())
...
class PerTurnEvalWorld(ModelChatWorld):
    def __init__(self, opt, agent, bots, context_info: Optional[dict] = None):
        ...
        print("this is block qualification: ", self.block_qualification)

       # if agent.mephisto_agent.get_worker().is_disqualified(self.block_qualification):
      #      logging.info("This worker is disqualified!")
        #    agent.mephisto_agent.get_worker().revoke_qualification(self.block_qualification)
    ...
    def parley(self):
        ...
        not_accepted_bot_response1 = human_choose_bot_response_act['task_data'][
            'not_accepted_bot_response1'
        ]
        not_accepted_bot_response2 = human_choose_bot_response_act['task_data'][
            'not_accepted_bot_response2'
        ]
        print("not_accepted_bot_response: ", not_accepted_bot_response2)
        
        not_accepted_bot_id1 = human_choose_bot_response_act['task_data'][
            'not_accepted_bot_id1'
        ]

        not_accepted_bot_id2 = human_choose_bot_response_act['task_data'][
            'not_accepted_bot_id2'
        ]
        ...
        self.bots[2].observe(accepted_bot_response)
        ...
        not_accepted_bot_utterance_data1 = {
            'text': not_accepted_bot_response1['text'].split('<br>')[0],
            'id': not_accepted_bot_id1,
        }
        not_accepted_bot_utterance_data2 = {
            'text': not_accepted_bot_response2['text'].split('<br>')[0],
            'id': not_accepted_bot_id2,
        }
        bot_utterance_data = {
            'agent_idx': 1,
            'accepted_bot_data': accepted_bot_utterance_data,
            'not_accepted_bot_data_1': not_accepted_bot_utterance_data1,
            'not_accepted_bot_data_2': not_accepted_bot_utterance_data2,
       ...
       # #translator = MarianTranslator(accepted_bot_response['text'])
      #  translator_to_en = MarianTranslatorFromTargetToEN(translator)
       ...
       # translated_in_message = translator_to_en.translate_question(accepted_bot_response["text"])
        ...
        if (
            ...
            with open(chat_data_path, 'w+', encoding='utf-8') as f_json:
                if self.opt['conversation_start_mode'] == 'hi_diff_lang':
                    data_str = json.dumps(self.final_chat_data, ensure_ascii=False)
                    try:
                        f_json.write(data_str)
                    except:
                        f_json.write(data_str.decode('utf8'))
                else:
                    data_str = json.dumps(self.final_chat_data)
                    f_json.write(data_str)
            print(
                f'{self.__class__.__name__}:{self.tag}: Data saved at '
                f'{chat_data_path} for models.'
            )
        ...
        logging.info(f"human_response_act: {human_response_act}")
        ...
        self.bots[2].observe(validate(human_response_act))
        ...
        bot_3_history = self.bots[2].model_agent.history.history_strings
        
        assert (
            bot_1_history == bot_2_history == bot_3_history
        ), f"The two bots' conversation histories are different.\nBot 1 history: {bot_1_history}\nBot 2 history: {bot_2_history} \nBot 3 history: {bot_3_history}"
        ...
        bot_3_response = self.bots[2].act()
        bot_3_response = Compatibility.maybe_fix_act(bot_3_response)
        ...
        if random.random() > 0.5:
            task_data = {
                ...
                'middle_bot_data': {
                    'middle_bot_id': self.bots[1].worker_id,
                    'middle_bot_response': bot_2_response,
                },
                'bottom_bot_data': {
                    'bottom_bot_id': self.bots[2].worker_id,
                    'bottom_bot_response': bot_3_response,
                },
            ...
            task_data = {
                ...
                'middle_bot_data': {
                    'middle_bot_id': self.bots[2].worker_id,
                    'middle_bot_response': bot_3_response,
                },
                'bottom_bot_data': {
    ...
    def get_final_chat_data(self) -> Dict[str, Any]:
        ...
        data = {
            ...
            'task_description': {
                ...
                'model_3_nickname': self.bots[2].worker_id,
                'model_3_file': self.bots[2].model_agent.opt.get('model_file'),
                'model_3_opt': self.bots[2].model_agent.opt,
            },
        }
    ...
    def _run_initial_turn(self) -> None:
        ...
        if self.opt['include_persona']:
            ...
            self.bots[2].observe(validate(message), increment_turn=False)

        if self.opt['conversation_start_mode'] == 'hi' or self.opt['conversation_start_mode'] == 'hi_diff_lang':
            ...
            if self.opt['conversation_start_mode'] == 'hi_diff_lang':
                logging.info('[Displaying "Hi!" in another language, only as per Meena task.]')
                if self.opt['start_language'] in hello_in_five_lang:
                    print(f"got {self.opt['start_language']} as the input language")
                    global greeting_in_lang # for bot agent first input!
                    greeting_in_lang = hello_in_five_lang.get(self.opt['start_language'], 'Hi!')
                
                print(f"greeting_in_lang is {greeting_in_lang}!")   
                human_first_msg = {
                'episode_done': False,
                'id': self.agent.agent_id,
                'text': greeting_in_lang,
                'fake_start': True,
                'agent_idx': 0,
                'task_data': {
            ...
            self.bots[2].observe(validate(human_first_msg))
            ...
            self.agent.observe({'text': '', 'task_data': task_data})
            print("this is task_data ", task_data)
    ...
    def shutdown(self):
        ...
        if self.chat_done:
            self.opt['run_statistics'][
                f'{self.bots[0].worker_id}:{self.bots[1].worker_id}:{self.bots[2].worker_id}'
            ] += 1
            logging.info(
...
def validate_onboarding(data):
    ...
    if status_message is None:
        return False
 #   print("this is status_message:", status_message)
    final_status = status_message.get('final_status')
 #   print(f"this is the onboarding status: {final_status}")
...
def get_translate_bot_worker(opt: Dict[str, Any], model_name: str) -> TranslatorPerTurnEvalTurkLikeAgent:
    """
    Return a bot agent.
    ...
    num_turns = opt['num_turns']
    bot_agent = create_agent_from_shared(shared_bot_agents[model_name])
    bot_worker = TranslatorPerTurnEvalTurkLikeAgent(
        opt,
        model_name=model_name,
        model_agent=bot_agent,
        num_turns=num_turns,
...
def make_world(opt, agents):
    ...
    model_1, model_2, model_3 = model_name.split(":")
    
    if model_1 == "blender_400M":
        # machine translation component for the baseline model
        bot_worker_1 = get_translate_bot_worker(opt=opt, model_name=model_1) #get_bot_worker
        bot_worker_2 = get_bot_worker(opt=opt, model_name=model_2)
        bot_worker_3 = get_bot_worker(opt=opt, model_name=model_3)
        
    elif model_2 == "blender_400M":
        bot_worker_1 = get_bot_worker(opt=opt, model_name=model_1) #get_bot_worker
        bot_worker_2 = get_translate_bot_worker(opt=opt, model_name=model_2)
        bot_worker_3 = get_bot_worker(opt=opt, model_name=model_3)
        
    elif model_3 == "blender_400M":
        bot_worker_1 = get_bot_worker(opt=opt, model_name=model_1) #get_bot_worker
        bot_worker_2 = get_bot_worker(opt=opt, model_name=model_2)
        bot_worker_3 = get_translate_bot_worker(opt=opt, model_name=model_3)
    ...
    return PerTurnEvalWorld(
        opt,
        agent=agents[0],
        bots=(bot_worker_1, bot_worker_2, bot_worker_3),
