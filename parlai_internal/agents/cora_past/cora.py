...
from parlai.agents.hugging_face.mt5 import mT5Agent
...
from parlai.agents.cora.modules import RagModel, T5RagModel, mT5RagModel
...
class mT5RagAgent(mT5Agent, BaseGenerationAgentMixin):
    @staticmethod
    def build_rag_model(opt: Opt, dictionary: DictionaryAgent) -> mT5RagModel:
        return mT5RagModel(opt, dictionary)
    ...
    def _generate(
        ...
        """
        Override since mT5 needs to call TGA generate.
        """
        return TorchGeneratorAgent._generate(
            self, batch, beam_size, max_ts, prefix_tokens
        )
...
class RagAgent(TransformerGeneratorRagAgent, BartRagAgent, T5RagAgent, mT5RagAgent):
    """
    RagAgent.
    ...
    _generation_agent: Union[
        Type[T5RagAgent], Type[mT5RagAgent], Type[BartRagAgent], Type[TransformerGeneratorRagAgent]
    ]
    ...
    def add_cmdline_args(
        ...
        mT5RagAgent.add_cmdline_args(parser, partial_opt=partial_opt)
