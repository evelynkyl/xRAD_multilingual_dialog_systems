...
from parlai.agents.hugging_face.mt5 import (
    ParlaimT5Encoder,
    ParlaimT5Decoder,
    build_mt5,
    set_device,
...
class mT5RagModel(RagModel):
    """
    mT5 For RAG.
    """

    def __init__(self, opt, dictionary, retriever_shared=None):
        opt['mt5'] = build_mt5(opt)
        if opt['mt5_model_parallel']:
            opt['mt5'].parallelize()
        else:
            opt['mt5'].deparallelize()
        super().__init__(opt, dictionary, retriever_shared)
        self.embedding_size = opt['mt5'].model_dim
        self.mt5 = opt.pop('mt5', None)
    ...
    def build_encoder(
        ...
        **kwargs,
        ...
        return RagModel.build_encoder(
            opt,
            encoder=opt['mt5'].get_encoder(),
            encoder_class=ParlaimT5Encoder,
            **kwargs,
        )
    ...
    def build_decoder(
        ...
        **kwargs,
        ...
        return RagModel.build_decoder(
            opt,
            decoder=opt['mt5'].get_decoder(),
            decoder_class=ParlaimT5Decoder,
            **kwargs,
        )
    ...
    @set_device
    def decoder_output(self, latent: torch.Tensor):
        tensor = latent * (self.mt5.model_dim ** -0.5)
        logits = self.mt5.lm_head(tensor)
        return logits
