...
def download(datapath):
    ddir = os.path.join(get_model_dir(datapath), 'cora')
    model_type = 'mgen_mdpr_cora_turn_do'
    version = 'v1.0'
    if not built(os.path.join(ddir, model_type), version):
        opt = {'datapath': datapath, 'model_type': model_type}
        fnames = ['model.tgz']
        download_models(
            opt, fnames, 'cora', version=version, use_model_type=True
        )
